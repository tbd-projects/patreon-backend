...
func (s *SuiteTestSesManager) SetupSuite() {
	...
	s.sessionsManager = CreateSessionsManager(s.mockIRepository)
}
...
func (s *SuiteTestSesManager) TestCreateSession() {
	...
	result, err := s.sessionsManager.CreateSession(userID)
...
func (s *SuiteTestSesManager) TestCheckSession() {
	...
	result, err := s.sessionsManager.CheckSession(uniqID)
	require.NoError(s.T(), err)
...
func (s *SuiteTestSesManager) TestDeleteSession() {
	...
	err := s.sessionsManager.DeleteSession(uniqID)
	require.NoError(s.T(), err)
}
