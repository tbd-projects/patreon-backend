...
type PostsUploadCoverHandler struct {
	postsUsecase usePosts.Usecase
	bh.BaseHandler
}

func NewPostsUploadCoverHandler(log *logrus.Logger, router *mux.Router, cors *app.CorsConfig,
	ucPosts usePosts.Usecase, manager sessions.SessionsManager) *PostsUploadCoverHandler {
	h := &PostsUploadCoverHandler{
...
// @Router /creators/{:creator_id}/posts/{:post_id}/cover [PUT]
func (h *PostsUploadCoverHandler) PUT(w http.ResponseWriter, r *http.Request) {
	var postId int64
	var ok bool
